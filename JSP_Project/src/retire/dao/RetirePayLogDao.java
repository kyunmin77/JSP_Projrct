...
public class RetirePayLogDao {
	...
	public List<RetirePayRequest> selectAllByRetireYear(Connection conn, int ret_year ) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"select e.emp_no, rp.ret_calc_type_mid, rp.ret_calc_type_retire, e.emp_type, e.name_kor, e.job, e.dept, e.state, e.hired_date, r.retired_date, rp.ret_other_date, rp.ret_other_name, rp.ret_other_cost, rp.ret_prize, rp.ret_notice, rp.ret_tax_free_pay, rp.ret_ad_pay, rp.ret_tax_free, "
					+ "rp.ret_3mon_sum, rp.ret_day_avg, rp.ret_pay, rp.ret_year, rp.ret_tax, rp.ret_income_tax, rp.ret_local_tax, rp.ret_realpay, rp.how_to_pay, rp.ret_payday from employee e left outer join retire r on (e.emp_no = r.emp_no) "
					+ "left outer join retire_payment rp on (r.emp_no = rp.emp_no) where rp.ret_year =?");
	...
	//해당 연도에 퇴직급여 지급 받은 사람들을 모두 불러오기
		public RetirePayRequest selectOneByRetireEmp_no(Connection conn, int emp_no) throws SQLException {
			PreparedStatement pstmt = null;
			ResultSet rs = null;

			try {
				pstmt = conn.prepareStatement(
						"select e.emp_no, rp.ret_calc_type_mid, rp.ret_calc_type_retire, e.emp_type, e.name_kor, e.job, e.dept, e.state, e.hired_date, r.retired_date, rp.ret_other_date, "
						+ "rp.ret_other_name, rp.ret_other_cost, rp.ret_prize, rp.ret_notice, rp.ret_tax_free_pay, rp.ret_ad_pay, rp.ret_tax_free, rp.ret_3mon_sum, rp.ret_day_avg, rp.ret_pay, "
						+ "rp.ret_year, rp.ret_tax, rp.ret_income_tax, rp.ret_local_tax, rp.ret_realpay, rp.how_to_pay, rp.ret_payday from employee e left outer join retire r on (e.emp_no = r.emp_no) "
						+ "left outer join retire_payment rp on (r.emp_no = rp.emp_no) where e.emp_no = ?");
