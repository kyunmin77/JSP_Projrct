...
//여기서부터 퇴직급여 테이블
		...
		String ret_calc_type = req.getParameter("ret_calc_type");
		String ret_calc_type_mid = (String) ((ret_calc_type.equals("中間精算")) ? "中間精算" : null);
		String ret_calc_type_retire = (String) ((ret_calc_type.equals("退職精算")) ? "退職精算" : null);

		System.out.println(ret_calc_type);
		System.out.println(ret_calc_type_mid);
		System.out.println(ret_calc_type_retire);
	 ...
	 //사원번호로 퇴직금 가져오기
	   private int selectRealPayByEmpNo(Connection conn, int emp_no) throws SQLException {
	      PreparedStatement pstmt = null;
	      ResultSet rs = null;
	      ...
	      try {
	         pstmt = conn.prepareStatement(
	               "select ret_realpay from retire_payment where emp_no = ?");
	         pstmt.setInt(1, emp_no);
	         rs = pstmt.executeQuery();

	         while (rs.next()) {
	            ret_realpay = Integer.parseInt(rs.getString("ret_realpay"));
	         }
	         return ret_realpay;
	      } finally {
	         JdbcUtil.close(rs);
	 ...
	 //퇴직 테이블에 퇴직금 저장하기
	   public void updateRealPay(Connection conn, int emp_no) throws SQLException {
	      PreparedStatement pstmt = null;

	      try {
	         pstmt = conn.prepareStatement("update retire set retire_cost = " + selectRealPayByEmpNo(conn,emp_no));
	         pstmt.executeUpdate();
