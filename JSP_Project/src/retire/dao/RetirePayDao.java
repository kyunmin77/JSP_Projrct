	...
	// 퇴직급여지급 사원선택 <모달>에서 전체 사원을 출력하기
		public List<RetirePayRequest> selectAll(Connection conn) throws SQLException {
			...
			try {
				...
				while (rs.next()) {
					...
					years_service = (int) ((retired_date - rs.getDate("hired_date").getTime()) / (365 * 24 * 60 * 60 * 1000L));
		...
		// 퇴직급여지급 사원선택 <모달>에서 전체 사원을 출력하기
				...
				public Date retiredDateByEmpNo(Connection conn, int emp_no) throws SQLException {
					...
					try {
						pstmt = conn.prepareStatement(
								"select retired_date from retire where emp_no=?");
						pstmt.setInt(1, emp_no);
						rs = pstmt.executeQuery();
				...
				public DayTerm getDayTerm(Connection conn, Date retired_date) throws SQLException {
					...
					try {
						pstmt = conn.prepareStatement("select add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1 as prev3_first,"
								+ " last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1) as prev3_last,"
								+ " last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)-(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1 as prev3_days,"
								+ " (last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1) as prev2_first,"
								+ " last_day(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1) as prev2_last,"
								+ " last_day(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1)-(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1)+1 as prev2_days,"
								+ " (last_day(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1)+1) as prev1_first,"
								+ " last_day(last_day(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1)+1) as prev1_last,"
								+ " last_day(last_day(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1)+1)-(last_day(last_day(add_months(to_date('" + retired_date + "', 'YYYY-MM-DD'),-3)+1)+1)+1)+1 as prev1_days,"
								+ " TRUNC(to_date('" + retired_date + "', 'YYYY-MM-DD'), 'MM') as this_first,"
								+ " to_date('" + retired_date + "', 'YYYY-MM-DD') as this_last,"
								+ " to_date('" + retired_date + "', 'YYYY-MM-DD')-TRUNC(to_date('" + retired_date + "', 'YYYY-MM-DD'), 'MM')+1 as this_days"
								+ " from dual");
						...
						while (rs.next()) {
							...
							dayTerm = new DayTerm(
									rs.getDate("prev3_first"),
									rs.getDate("prev3_last"),
									rs.getInt("prev3_days"),
									rs.getDate("prev2_first"),
									rs.getDate("prev2_last"),
									rs.getInt("prev2_days"),
									rs.getDate("prev1_first"),
									rs.getDate("prev1_last"),
									rs.getInt("prev1_days"),
									rs.getDate("this_first"),
									rs.getDate("this_last"),
									rs.getInt("this_days"));
						}
						return dayTerm;
					} finally {
